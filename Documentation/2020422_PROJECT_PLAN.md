...
├── PHASE_0_PROMPTS.md              ← prompts for sub-task conversations
├── .gitignore
├── .editorconfig
...
## 9. Working style — how this project is built with AI assistance

This project is built in collaboration with an AI assistant (Claude). The workflow
is deliberate — it's not "ask Claude to write the whole thing," it's "use Claude as
a mentor while I learn C++ backend development deeply."

### Multi-conversation pattern

- **One master conversation** — for architecture, decision-log updates, roadmap
  revisions, and "stepping back" moments. This document lives here.
- **One conversation per sub-task** — focused, deep, disposable. Starts fresh with
  a bootstrap prompt, does one thing, ends with a commit.
- **`PROJECT_PLAN.md` is the handoff document.** New conversations start by reading
  it; they're up to speed in seconds. This file is institutional memory.
- **`PHASE_0_PROMPTS.md`** (and future phase-prompt files) pre-stage the sub-task
  conversations so we can work through the roadmap without coming back to the
  master conversation between every step.

### Rules for the workflow

1. **Every sub-task ends with a git commit.** Git history is the second handoff mechanism.
2. **Decisions made in sub-task conversations get back-ported to the decision log
   in this file** before closing the tab. Don't let choices live only in chat history.
3. **If a sub-task exposes a problem with a prior architectural decision, come back
   to the master conversation.** Update the plan, then start the next sub-task with
   the corrected context.
4. **Attach `PROJECT_PLAN.md` to every new sub-task conversation.** Don't paste — attach.
5. **Include `git log --oneline -10` in every new sub-task prompt.** Fast orientation.

### Learning mode, not delivery mode

Every sub-task prompt tells Claude to:
- Act as a senior engineer mentoring me.
- Explain *every* decision, not just produce files.
- Pause at decision points and ask for my input.
- Push back if I suggest something questionable.

If a conversation starts feeling like "Claude does everything while I nod," the
prompt wasn't strict enough. Fix the prompt template in `PHASE_0_PROMPTS.md`
and use the fixed version from there on.
...
| 2026-04-21 | Multi-conversation workflow with sub-task prompts | Keep each conversation focused; PROJECT_PLAN.md is the handoff document |
| 2026-04-21 | Phase 0 sub-tasks pre-planned in PHASE_0_PROMPTS.md | Enables working through the roadmap without returning to master conversation between steps |
